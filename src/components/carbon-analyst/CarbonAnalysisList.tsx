...
export function CarbonAnalysisList() {
  ...
  if (selectedAnalysisId) {
    return <CarbonAnalysisReport analysisId={selectedAnalysisId} onNewAnalysis={() => setSelectedAnalysisId(null)} onViewHistory={() => setSelectedAnalysisId(null)} />;
  }
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div><CardTitle>Analysis History</CardTitle><CardDescription>View and manage your past carbon credit offtake analyses</CardDescription></div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground mt-1">Upload a carbon credit offtake agreement to get started</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                ...
                {filteredAnalyses.map(analysis => (
                  <TableRow key={analysis.id} className="cursor-pointer hover:bg-muted/50">
                    <TableCell className="font-medium" onClick={() => setSelectedAnalysisId(analysis.id)}>
                      <div><p>{analysis.project_name}</p><p className="text-xs text-muted-foreground">{analysis.document_file_name}</p></div>
                    </TableCell>
                    <TableCell onClick={() => setSelectedAnalysisId(analysis.id)}>
                      <Badge variant="outline">{analysis.analysis_type === 'carbon_vs_bible' ? 'vs Knowledge' : 'vs Term Sheet'}</Badge>
                    </TableCell>
                    <TableCell onClick={() => setSelectedAnalysisId(analysis.id)}>
                      {analysis.carbon_type ? <Badge variant="outline" className="text-xs">{CARBON_PROJECT_TYPES.find(t => t.id === analysis.carbon_type)?.label || analysis.carbon_type}</Badge> : '—'}
                    </TableCell>
                    <TableCell onClick={() => setSelectedAnalysisId(analysis.id)}>
                      <Badge variant="secondary">{analysis.perspective === 'buyer' ? 'Buyer' : 'Seller'}</Badge>
                    </TableCell>

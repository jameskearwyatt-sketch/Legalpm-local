...
import { useITSupplyLearnings, ITSupplyLearning } from '@/lib/hooks/useITSupplyLearnings';
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from '@/components/ui/collapsible';
import { format } from 'date-fns';

export function ITSupplyLearningsTab() {
  const { learnings, isLoading, deleteLearning, toggleActive } = useITSupplyLearnings();
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(new Set());

  const learningsByCategory: Record<string, ITSupplyLearning[]> = {};
  ...
  if (learnings.length === 0) return <Card><CardContent className="py-12 text-center"><Brain className="h-12 w-12 mx-auto text-muted-foreground/50 mb-4" /><h3 className="text-lg font-medium mb-2">No AI Learnings Yet</h3><p className="text-muted-foreground max-w-md mx-auto">When you provide feedback on analysis positions using the "Teach AI" button, those corrections will be stored here and applied to future analyses.</p></CardContent></Card>;

  return (
    <div className="space-y-6">
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><Lightbulb className="h-5 w-5 text-amber-500" /> AI Learnings</CardTitle><CardDescription>Corrections and feedback to improve IT supply analysis accuracy</CardDescription></CardHeader><CardContent><div className="flex items-center gap-4"><div className="flex items-center gap-2"><Badge variant="default">{activeCount} active</Badge>{learnings.length - activeCount > 0 && <Badge variant="secondary">{learnings.length - activeCount} inactive</Badge>}</div><span className="text-sm text-muted-foreground">across {sortedCategories.length} categories</span></div></CardContent></Card>

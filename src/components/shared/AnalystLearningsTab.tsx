...
interface AnalystLearningsTabProps<T extends BaseAnalystLearning> {
  /** The learnings hook (e.g. `useTollingLearnings`) already invoked. */
  hook: LearningsHookResult<T>;
  /** Short description shown under the header, e.g. "tolling analysis accuracy". */
  description: string;
}

/**
 * Shared Learnings tab UI used by the 4 simple analyst tools
 * (Tolling, Carbon, IT Supply, Cloud Compute). Takes the already-invoked
 * hook result so callers retain full type safety over their learning shape.
 */
export function AnalystLearningsTab<T extends BaseAnalystLearning>({
  hook,
  description,
}: AnalystLearningsTabProps<T>) {
  const { learnings, isLoading, deleteLearning, toggleActive } = hook;
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(new Set());

  const learningsByCategory: Record<string, T[]> = {};
  for (const l of learnings) {
    if (!learningsByCategory[l.category]) learningsByCategory[l.category] = [];
    learningsByCategory[l.category].push(l);
  }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Lightbulb className="h-5 w-5 text-amber-500" />AI Learnings
          </CardTitle>
          <CardDescription>Corrections and feedback to improve {description}</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <CollapsibleTrigger asChild>
                  <button className="flex items-center justify-between w-full p-3 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
                    <div className="flex items-center gap-2">
                      ...
                      {activeInCategory > 0 && (
                        <Badge variant="default" className="text-xs">{activeInCategory} active</Badge>
                      )}
                    </div>
                  </button>
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <div className="mt-2 space-y-2 pl-6">
                    {categoryLearnings.map(learning => (
                      <div
                        key={learning.id}
                        className={`p-4 border rounded-lg space-y-3 ${!learning.is_active ? 'opacity-60' : ''}`}
                      >
                        <div className="flex items-start justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <Switch
                              ...
                              onCheckedChange={(v) => toggleActive.mutate({ id: learning.id, is_active: v })}
                            />
                            <AlertDialog>
                        ...
                        <div className="space-y-1">
                          <span className="text-xs font-medium text-primary">Corrected:</span>
                        ...
                        {learning.correction_reason && (
                          <div className="space-y-1">
                            <span className="text-xs font-medium text-amber-600 dark:text-amber-400">Reason:</span>

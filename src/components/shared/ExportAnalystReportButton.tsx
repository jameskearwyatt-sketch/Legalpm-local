...
function renderPositionHtml(p: ExportPosition): string {
  ...
  const sourceBadge = p.sourceText
    ? `<span style="display: inline-block; padding: 2px 6px; font-size: 9pt; font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; background-color: #f3f4f6; color: #6b7280; border-radius: 3px;">${escapeHtml(p.sourceText)}</span>`
    : '';

  const summaryLines = p.positionSummary.split('\n').filter(l => l.trim());
  const summaryHtml = summaryLines
    .map(line => `<p style="font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; font-size: 10.5pt; color: #1f2937; margin: 4px 0; line-height: 1.55;">${escapeHtml(line)}</p>`)
    .join('');
  ...
  const comparisonHtml = p.comparisonPosition
    ? `<div style="border-left: 3px solid #c7d2fe; padding: 6px 10px; margin: 8px 0; background-color: #f5f3ff;">
         <p style="font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; font-size: 9pt; color: #6b7280; font-weight: 600; margin: 0 0 3px 0; text-transform: uppercase; letter-spacing: 0.4px;">Comparison</p>
         <p style="font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; font-size: 10pt; color: #4c1d95; font-style: italic; margin: 0;">${escapeHtml(p.comparisonPosition)}</p>
       </div>`
    : '';

  const cleanedNotes = stripMarkers(p.varianceNotes);
  const notesHtml = cleanedNotes
    ? `<p style="font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; font-size: 9.5pt; color: #6b7280; margin: 6px 0 0 0; font-style: italic;">${escapeHtml(cleanedNotes)}</p>`
    : '';
  ...
  return `
    <div style="page-break-inside: avoid; border: 1px solid #e5e7eb; border-radius: 6px; padding: 12px 14px; margin-bottom: 10px; background-color: #ffffff;">
      <p style="font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; font-size: 11.5pt; font-weight: 700; color: #111827; margin: 0 0 8px 0;">${escapeHtml(p.category)}</p>
  ...
  const groupsHtml = payload.positionsByGroup
    ...
    .map((g, idx) => `
      <div style="page-break-inside: avoid; margin-top: ${idx === 0 ? '0' : '24px'};">
        <h2 style="font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; font-size: 15pt; color: #111827; border-bottom: 2px solid #1e40af; padding-bottom: 5px; margin: 0 0 14px 0;">
          ${escapeHtml(g.group)}
...
<div class="Section1">
  <div style="text-align: center; padding-top: 100px; padding-bottom: 60px;">
    <p style="font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; font-size: 11pt; color: #6b7280; margin: 0 0 8px 0; text-transform: uppercase; letter-spacing: 2px;">${escapeHtml(payload.analystTitle)} — Analysis Report</p>
    <h1 style="font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; font-size: 26pt; color: #111827; font-weight: 700; margin: 0 0 14px 0; letter-spacing: -0.5px;">${escapeHtml(payload.projectName)}</h1>
    <div style="width: 80px; height: 3px; background-color: #1e40af; margin: 0 auto 20px auto;"></div>
    <p style="font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; font-size: 11pt; color: #4b5563; margin: 0 0 4px 0;">${now}</p>
  ...
  <div style="margin-bottom: 28px;">
    <h2 style="font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; font-size: 15pt; color: #111827; border-bottom: 2px solid #1e40af; padding-bottom: 5px; margin: 0 0 14px 0;">Analysis Overview</h2>
    ...
    <p style="font-family: 'Manrope', 'Segoe UI', Arial, sans-serif; font-size: 10pt; color: #6b7280; margin: 0;">Analysed: ${analysedAt}</p>
  </div>

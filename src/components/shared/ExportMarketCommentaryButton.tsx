  ...
  const sectionHtml = results.map((r, idx) => {
    const renderPosition = (data: { title: string; summary: string; points: string[] }, color: string) => `
      <div style="margin-bottom: 18px;">
        <h4 style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 13pt; color: ${color}; margin: 0 0 6px 0; text-transform: uppercase; letter-spacing: 0.5px;">${data.title}</h4>
        <p style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 11pt; color: #374151; font-style: italic; margin: 0 0 8px 0;">${data.summary}</p>
        <ul style="margin: 0; padding-left: 20px;">
          ${data.points.map(pt => `<li style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 10.5pt; color: #1f2937; margin-bottom: 5px; line-height: 1.5;">${pt}</li>`).join('')}
        </ul>
      </div>
    `;
    ...
    const insightsHtml = r.keyInsights?.length ? `
      <div style="background-color: #f0f9ff; border-left: 3px solid #3b82f6; padding: 12px 16px; margin: 16px 0;">
        <p style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 10pt; font-weight: bold; color: #1e40af; margin: 0 0 6px 0;">KEY INSIGHTS</p>
        ${r.keyInsights.map(ins => `<p style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 10pt; color: #1e3a5f; margin: 3px 0;">💡 ${ins}</p>`).join('')}
      </div>
    ` : '';

    const confidenceHtml = r.confidenceNote ? `
      <p style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 9pt; color: #6b7280; font-style: italic; margin-top: 12px;">📊 ${r.confidenceNote}</p>
    ` : '';

    return `
      <div style="page-break-inside: avoid; margin-bottom: 36px;">
        <h2 style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 16pt; color: #111827; border-bottom: 2px solid #1e40af; padding-bottom: 6px; margin: 0 0 4px 0;">
          ${idx + 1}. ${r.category}
        </h2>
        <p style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 10pt; color: #6b7280; margin: 0 0 18px 0;">Based on ${r.dealCount} banked precedent${r.dealCount !== 1 ? 's' : ''}</p>
...
<div class="Section1">
  ...
  <div style="text-align: center; padding-top: 120px; padding-bottom: 80px;">
    <h1 style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 28pt; color: #111827; font-weight: 700; margin: 0 0 12px 0; letter-spacing: -0.5px;">MARKET COMMENTARY</h1>
    <div style="width: 80px; height: 3px; background-color: #1e40af; margin: 0 auto 20px auto;"></div>
    <p style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 14pt; color: #4b5563; margin: 0 0 8px 0;">${analystTitle}</p>
    <p style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 11pt; color: #9ca3af; margin: 0 0 4px 0;">${now}</p>
  ...
  <!-- Table of Contents -->
  <h2 style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 16pt; color: #111827; border-bottom: 2px solid #1e40af; padding-bottom: 6px; margin-bottom: 16px;">Contents</h2>
  <ol style="padding-left: 20px;">
    ${results.map((r, i) => `<li style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; font-size: 11pt; color: #374151; margin-bottom: 6px;">${r.category} <span style="color: #9ca3af;">(${r.dealCount} precedent${r.dealCount !== 1 ? 's' : ''})</span></li>`).join('')}
  </ol>

...
import { useTollingLearnings, TollingLearning } from '@/lib/hooks/useTollingLearnings';
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from '@/components/ui/collapsible';
import { format } from 'date-fns';

export function TollingLearningsTab() {
  const { learnings, isLoading, deleteLearning, toggleActive } = useTollingLearnings();
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(new Set());

  const learningsByCategory: Record<string, TollingLearning[]> = {};
  for (const learning of learnings) {
    if (!learningsByCategory[learning.category]) learningsByCategory[learning.category] = [];
  ...
  if (isLoading) {
    return <Card><CardContent className="py-8 flex justify-center"><Loader2 className="h-8 w-8 animate-spin text-primary" /></CardContent></Card>;
  }
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Corrections and feedback to improve tolling analysis accuracy</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <Badge variant="default">{activeCount} active</Badge>
              {learnings.length - activeCount > 0 && <Badge variant="secondary">{learnings.length - activeCount} inactive</Badge>}
            </div>
...
function LearningCard({ learning, onToggleActive, onDelete }: {
  learning: TollingLearning;
  ...
  return (
    <div className={`p-4 border rounded-lg space-y-3 ${!learning.is_active ? 'opacity-60' : ''}`}>
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <Switch checked={learning.is_active} onCheckedChange={onToggleActive} />
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>This will permanently remove this correction from the AI's memory.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>

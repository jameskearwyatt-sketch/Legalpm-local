...
export function TollingTeachFeedbackDialog({
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      // For now, store the feedback as a learning directly
      // A dedicated tolling feedback edge function can be added later
      ...
      // Update the position text in the database
      const { error: updateError } = await supabase
        .from('tolling_extracted_positions')
        .update({ position_summary: corrected } as any)
        .eq('id', position.id);

      if (!updateError && onPositionUpdated) {
        onPositionUpdated(corrected);
      }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Provide feedback to correct this analysis. The AI will remember this for future tolling analyses.
          </DialogDescription>
        </DialogHeader>

...
export function useCarbonPrecedentBank() {
  ...
  const bankPositions = useMutation({
    mutationFn: async (newPrecedents: Omit<CarbonPrecedent, 'id' | 'banked_at'>[]) => {
      ...
      const { data, error } = await supabase
        ...
        .select();
      if (error) throw error;
      const rows = data as unknown as CarbonPrecedent[];
      for (const row of rows) {
        ...
        void embedAndStore('carbon', 'precedent', row.id, embedSource);
      }
      return rows;
    },
    onSuccess: () => {
  ...
  ): Promise<{ precedents: CarbonPrecedent[]; usedSemanticRetrieval: boolean }> => {
    const pool = onlyGoldStandard ? goldStandardPrecedents : (precedents || []);
    const embedding = await embedText(queryText);
    const matched = await matchPrecedents<{ id: string }>('carbon', embedding, k, 0.3, onlyGoldStandard);
    if (matched && matched.length > 0) {
      const byId = new Map(pool.map(p => [p.id, p]));
      const hydrated = matched.map(m => byId.get(m.id)).filter((p): p is CarbonPrecedent => !!p);

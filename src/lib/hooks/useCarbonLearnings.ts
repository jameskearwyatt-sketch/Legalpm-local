...
export function useCarbonLearnings() {
  const { user } = useAuth();
  const queryClient = useQueryClient();

  const { data: learnings, isLoading, error } = useQuery({
    queryKey: ['carbon-learnings', user?.id],
    queryFn: async () => {
      if (!user) return [];
      const { data, error } = await supabase
        .from('carbon_learnings')
        .select('*')
  ...
  const createLearning = useMutation({
    mutationFn: async (learning: Omit<CarbonLearning, 'id' | 'created_at' | 'updated_at'>) => {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['carbon-learnings'] });
  ...
  const deleteLearning = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('carbon_learnings').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['carbon-learnings'] });

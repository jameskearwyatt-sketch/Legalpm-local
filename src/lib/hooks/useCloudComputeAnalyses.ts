...
export function useCloudComputeAnalyses() {
  ...
  const createAnalysisWithPositions = useMutation({
    mutationFn: async (args: {
      analysis: Omit<CloudComputeAnalysis, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'is_agreed' | 'agreed_at'>;
      positions: Omit<CloudComputeExtractedPosition, 'id' | 'analysis_id' | 'user_id' | 'created_at'>[];
    }) => {
      if (!user) throw new Error('Not authenticated');
      const { data, error } = await supabase.rpc(
        'create_cloud_compute_analysis_with_positions' as never,
        {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cloud-compute-analyses'] });
      queryClient.invalidateQueries({ queryKey: ['cloud-compute-positions'] });
    },

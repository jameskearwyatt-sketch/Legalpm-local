...
export function useITSupplyPrecedentBank() {
  ...
  const bankPositions = useMutation({
    mutationFn: async (newPrecedents: Omit<ITSupplyPrecedent, 'id' | 'banked_at'>[]) => {
      ...
      const { data, error } = await supabase
        ...
        .select();
      if (error) throw error;
      const rows = data as unknown as ITSupplyPrecedent[];
      for (const row of rows) {
        ...
        void embedAndStore('it_supply', 'precedent', row.id, embedSource);
      }
      return rows;
    },
    onSuccess: () => {
  ...
  ): Promise<{ precedents: ITSupplyPrecedent[]; usedSemanticRetrieval: boolean }> => {
    const pool = onlyGoldStandard ? goldStandardPrecedents : (precedents || []);
    const embedding = await embedText(queryText);
    const matched = await matchPrecedents<{ id: string }>('it_supply', embedding, k, 0.3, onlyGoldStandard);
    if (matched && matched.length > 0) {
      const byId = new Map(pool.map(p => [p.id, p]));
      const hydrated = matched.map(m => byId.get(m.id)).filter((p): p is ITSupplyPrecedent => !!p);

...
export function useTollingAnalyses() {
  ...
  const createAnalysisWithPositions = useMutation({
    mutationFn: async (args: {
      analysis: Omit<TollingAnalysis, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'is_agreed' | 'agreed_at'>;
      positions: Omit<TollingExtractedPosition, 'id' | 'analysis_id' | 'user_id' | 'created_at'>[];
    }) => {
      if (!user) throw new Error('Not authenticated');
      const { data, error } = await supabase.rpc(
        'create_tolling_analysis_with_positions' as never,
        {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tolling-analyses'] });
      queryClient.invalidateQueries({ queryKey: ['tolling-positions'] });
    },
...
export function useTollingPrecedentBank() {
  ...
  const bankPositions = useMutation({
    mutationFn: async (newPrecedents: Omit<TollingPrecedent, 'id' | 'banked_at'>[]) => {
      ...
      const { data, error } = await supabase
        ...
        .select();
      if (error) throw error;
      const rows = data as unknown as TollingPrecedent[];
      for (const row of rows) {
        ...
        void embedAndStore('tolling', 'precedent', row.id, embedSource);
      }
      return rows;
    },
    onSuccess: () => {
  ...
  ): Promise<{ precedents: TollingPrecedent[]; usedSemanticRetrieval: boolean }> => {
    const pool = onlyGoldStandard ? goldStandardPrecedents : (precedents || []);
    const embedding = await embedText(queryText);
    const matched = await matchPrecedents<{ id: string }>('tolling', embedding, k, 0.3, onlyGoldStandard);
    if (matched && matched.length > 0) {
      const byId = new Map(pool.map(p => [p.id, p]));
      const hydrated = matched.map(m => byId.get(m.id)).filter((p): p is TollingPrecedent => !!p);

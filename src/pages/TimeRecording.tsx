...
import type { BudgetLineItem, WorkItemAllocation, GridRowEntry, DayOutputEntry, DayOutput, Step } from './timeRecordingTypes';
import { NON_CHARGEABLE_CODES } from './timeRecordingTypes';
import { formatOutputForClipboard, formatOutputAsHtml, formatOutputForEmail } from './timeRecordingFormatters';
...
export default function TimeRecording() {
  ...
  const copyToClipboard = async () => {
    if (!processedOutput) return;
    const plainText = formatOutputForClipboard(processedOutput);
    const htmlContent = formatOutputAsHtml(processedOutput);
  ...
  const sendTimeRecordingEmail = () => {
    if (!processedOutput) return;
    const emailBody = formatOutputForEmail(processedOutput);
    const dateRange = processedOutput.length > 0 
